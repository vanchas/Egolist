...
export default function MainListControl({ changeComponent, visibleComponent, sortDesires, sortOffers }) {
  ...
  return (
    <div>
      ...
      <div className={s.list_control}>
        <div>
          <span onClick={() => { changeComponent('desires') }} className={`${visibleComponent === 'desires' ? s.active_black : null}`}>Желания</span>
          <span onClick={() => changeComponent('offers')} className={`${visibleComponent === 'offers' ? s.active_black : null}`}>Предложения</span>
        </div>
