...
const NavComponent = ({ locations, searchInfo, filterOffers, filterDesires }) => {
    ...
    <div className={s.navbar_nav}>
      <Link href="/">
        <a className={`${s.navbar_brand}`}>
          <img src={MainLogo} alt="EGOLIST" className={`mx-auto ${s.main_logo}`} />
        </a>
      </Link>
      ...
      <form className={`${s.header_form} form-inline my-2 my-lg-0`}>
        <input className={`font-weight-bold border-0 form-control text-dark ${s.search_input}`} type="search" placeholder="Начните вводить поисковый запрос" aria-label="Search" onChange={e => setSearchValue(e.target.value)} />
        <select className={`font-weight-bold ${s.search_select} border-0 form-control text-dark`} onChange={(e) => {
          filterOffers('region_id', e.target.value);
          filterDesires('region_id', e.target.value);
        }}>
          <option value="default" hidden>Вся украина</option>
          {locations && locations.length ?
            locations.map((l, i) => (
              <option value={l.id} key={i}>{l.name_ru}</option>
            )) : null}
        </select>
        <button
