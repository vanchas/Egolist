...
export const searchInfo = (
  ...
  // console.log(search_field, region_id, city_id, category_ids, subcategory_ids);
  const url = `/desires/search?search_field=${search_field}&region_id=${region_id}&city_id=${city_id}&category_ids[]=${category_ids}&subcategory_ids[]=${subcategory_ids}`;
...
export const getDesireById = (id: number | string) => async (
  dispatch: Function
) => {
...
export const getOffersByDesireId = (desire_id: string | number) => async (
  ...
  HttpRequest.execute(`/filter_offer`, "POST", "application/json", {
    desire_id,
  })
    .then((res) => {
...
export const getDesiresByCategory = (category_id: string | number) => async (
  ...
  HttpRequest.execute(`/filter_desire`, "POST", "application/json", {
