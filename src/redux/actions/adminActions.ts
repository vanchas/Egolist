import {
  ...
  REFUSE_USER_VERIFICATION,
  DELETE_PHOTO_VERIFY_EXAMPLE,
  SEARCH_ALL_USERS,
  GET_ALL_USERS,
...
export const updateComplaint = (
  ...
  HttpRequest.execute(`/complaints/change/${id}`, "POST", "application/json", {
    status_id,
  })
    .then((data) => {
...
export const deleteComplaint = (id: number | string) => async (
  dispatch: Function
) => {
  HttpRequest.execute(`/complaints/${id}`, "DELETE")
    .then((data) => {
      return dispatch({ type: DELETE_COMPLAINT, payload: id });
    })
    .catch((err) => console.error("Error: ", err));
};

export const createCategory = (name: string) => async (dispatch: Function) => {
  HttpRequest.execute(`/categories/create`, "POST", "application/json", {
    name,
  })
    .then((res) => {
...
export const editCategory = (id: number | string, name: string) => async (
  dispatch: Function
) => {
  HttpRequest.execute(`/categories/edit/${id}`, "POST", "application/json", {
    name,
  })
    .then((res) => {
...
export const deleteCategory = (id: number | string) => async (
  dispatch: Function
) => {
...
export const createSubcategory = (
  ...
  HttpRequest.execute(
    `/categories/sub/${categoryId}/create`,
    "POST",
    "application/json",
    { name }
  )
...
export const editSubcategory = (
  ...
  HttpRequest.execute(
    `/categories/sub/${categoryId}/edit/${id}`,
    "POST",
    "application/json",
    { name }
  )
...
export const deleteSubcategory = (id: number | string) => async (
  dispatch: Function
) => {
...
export const getAllUsers = (page: any) => async (dispatch: Function) => {
...
export const searchAllUsers = (search: string) => async (
  dispatch: Function
) => {
...
export const allowUserVerification = (id: any) => async (
  dispatch: Function
) => {
...
export const refuseUserVerification = (id: any, message: string) => async (
  dispatch: Function
) => {
  // admin/refuse/{id} - post, отказ от верификации, передаешь мне поле "message", не менее 200 символов с причиной отказа
  HttpRequest.execute(`/admin/refuse/${id}`, "POST", "application/json", {
    message,
  })
    .then((data) => {
      if (data.errors) {
...
export const sortAllUsersList = (id: any) => async (
  dispatch: Function
) => {
...
export const deletePhotoVerifyExample = (photo: string) => async (
  dispatch: Function
) => {
  // photo/delete - delete, в реквесте имя фотки
  HttpRequest.execute(`/admin/photo/delete`, "POST", "application/json", {
    example: photo,
  })
    .then((data) => {
...
export const uploadPhotoVerifyExample = (
  photoFirst: any,
  photoSecond: any
) => async (dispatch: Function) => {
